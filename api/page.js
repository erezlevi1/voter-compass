...
const PAGES = {
  async parties(host) {
    ...
    const rows = P.map(p => `<tr><td><strong>${esc(p.name)}</strong></td><td>${p.ballot ? `<span class="ballot">${esc(p.ballot)}</span>` : '<span style="color:var(--ink-3)">טרם נקבעה</span>'}</td><td>${esc(p.leader)}</td><td>${esc(p.bloc)}</td><td>${p.seats > 0 ? p.seats : '—'}</td></tr>`).join('');
    const cards = P.map(p => `<div class="card"><h3>${esc(p.name)}${p.ballot ? ` <span class="ballot">${esc(p.ballot)}</span>` : ''}</h3><p style="margin:0"><strong>${esc(p.leader)}</strong> · גוש ${esc(p.bloc)}${p.seats > 0 ? ` · ${p.seats} מנדטים בכנסת ה-25` : ' · רשימה חדשה'}<br>${esc(p.essence)}</p></div>`).join('');
    return {
...
${merged.length ? `<h2>רשימות שהתמזגו</h2><p>${merged.map(p => esc(p.name)).join(', ')} — אינה רצה עצמאית בבחירות 2026.</p>` : ''}
...
<p style="margin:9px 0 0"><a href="${BASE}/#/issue/${esc(it.explainer)}">לעמוד המלא של הנושא ←</a></p></div>`;
    }).join('');
    return {
